...
function SelectorList(props: IProps) {
	...
	const handleSelectorToggle = (e: React.ChangeEvent<HTMLInputElement>, s: string) => {
	...
	const selector = (s: string, i: number) => {
		return (
			<li key={i} className={styles.rule}>
				<label><input type="checkbox" onChange={(e) => handleSelectorToggle(e, s)}
							  checked={current === s}/>{s}</label>
			</li>
		);
	};
