...
function drawOverlay(points: IPoint[], canvasRef: React.MutableRefObject<HTMLCanvasElement>) {
	...
	for (const point of points) {
		const {x, y} = point;
		const rect = [
			(x * scale - (markerSize / 2)), (y * scale - (markerSize / 2)), markerSize, markerSize
		] as const;
...
function findPoint(mouseDownX: number, mouseDownY: number, points: IPoint[]): IPoint | null {
...
function scaleCursor(x: number, y: number, current: HTMLCanvasElement): { scaledX: number, scaledY: number } {
	const scale = getScale(current);
	return {scaledX: x / scale, scaledY: y / scale};
}

function onMouseUp(e: React.MouseEvent, point: IPoint, canvas: HTMLCanvasElement, callback: (changeOptions: ChangeOptions) => void) {
...
function PreviewPane(props: IProps) {
	...
	function handleMouseDown(e: React.MouseEvent<HTMLCanvasElement>) {
		...
		if (match) {
			...
			e.currentTarget.addEventListener("mouseup", (mouseUpE) => onMouseUp(mouseUpE as unknown as React.MouseEvent, match, canvasRef.current, props.onChange), {once: true});
		}
	}
	...
	return (
		<div className={styles.previewPane + " " + (isDark ? styles.dark : styles.light)}>
			<div className={styles.title}><h2>Preview</h2><label>Dark: <input type="checkbox" checked={isDark}
																			  onChange={(e) => setIsDark(e.currentTarget.checked)}/></label>
			</div>
			<h3>&lt;svg&gt;</h3>
			<div className={styles.livePreview}>
				...
				></div>
				<canvas ref={canvasRef} onMouseDown={(e) => handleMouseDown(e)}></canvas>
			</div>
			<h3>&lt;img&gt;</h3>
